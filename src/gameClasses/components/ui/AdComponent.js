	...
	playCallback: function (data, clientId) {
		...
		if (taro.script) { // do not send trigger for neutral player
			if (data.status === 'completed') {
				taro.trigger.fire('adPlayCompleted', { playerId: player.id() });
			} else if (data.status === 'skipped') {
				taro.trigger.fire('adPlaySkipped', { playerId: player.id() });
			} else if (data.status === 'blocked') {
				taro.trigger.fire('adPlayBlocked', { playerId: player.id() });
				taro.server.userAdStats[distinctId].adFailAttempts.push(taro.currentTime());
			} else if (data.status === 'failed') {
				taro.trigger.fire('adPlayFailed', { playerId: player.id() });
				taro.server.userAdStats[distinctId].adFailAttempts.push(taro.currentTime());
			}
		}

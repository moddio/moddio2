...
const dyn_static_exitVelocity = function (a_entity, overlapN) {
	//
	let aVel = new taro.physics.simulation.crash.Vector(a_entity._velocity.x, a_entity._velocity.y);
...
const dyn_dyn_exitVelocities = function (a_entity, b_entity, overlapN) {
	...
	let aVel = new taro.physics.simulation.crash.Vector(a_entity._velocity.x, a_entity._velocity.y);
	let bVel = new taro.physics.simulation.crash.Vector(b_entity._velocity.x, b_entity._velocity.y);
	temp = aVel;
